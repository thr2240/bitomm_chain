...
KEYS[0]="bitommFoundation"

CHAINID="bitomm_3131-1"
MONIKER="bitommteam"
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="bitomm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["amount"]="1000"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	jq '.app_state["incentives"]["params"]["enable_incentives"]=false' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	sed -i 's/address = "127.0.0.1:8545"/address = "0.0.0.0:8545"/g' ~/.bitommd/config/app.toml
	sed -i 's/ws-address = "127.0.0.1:8546"/ws-address = "0.0.0.0:8546"/g' ~/.bitommd/config/app.toml
	# sed -i 's/tracer = ""/tracer = "json"/g' ~/.bitommd/config/app.toml
	...
	# sed -i 's/address = "tcp:\/\/0.0.0.0:1317"/address = "tcp:\/\/127.0.0.1:1317"/g' ~/.bitommd/config/app.toml
	# sed -i 's/address = "0.0.0.0:9090"/address = "127.0.0.1:9090"/g' ~/.bitommd/config/app.toml
	# sed -i 's/address = "0.0.0.0:9091"/address = "127.0.0.1:9091"/g' ~/.bitommd/config/app.toml
	...
	sed -i '/\[api\]/,+3 s/enable = false/enable = true/' ~/.bitommd/config/app.toml
	sed -i 's/swagger = false/swagger = true/g' ~/.bitommd/config/app.toml
	sed -i 's/enabled-unsafe-cors = false/enabled-unsafe-cors = true/g'  ~/.bitommd/config/app.toml
	# sed -i 's/enable-indexer = false/enable-indexer = true/g' ~/.bitommd/config/app.toml
	sed -i 's/api = "eth,net,web3"/api = "eth,txpool,personal,net,debug,web3,pubsub,trace"/g' ~/.bitommd/config/app.toml
	sed -i 's/pruning = "default"/pruning = "nothing"/g' ~/.bitommd/config/app.toml

	# Allocate genesis accounts (cosmos formatted addresses)
	
	bitommd add-genesis-account ${KEYS[0]} 10800000000000000000000000abitomm --keyring-backend $KEYRING --home "$HOMEDIR"


	# bc is required to add these big numbers
	total_supply=$(echo "10800000000000000000000000" | bc)
